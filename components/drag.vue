<template>
	<view class="drag-box" :containerSize="[containerSize]">
		<template v-if="controlsPositionArray.length !== 0">
			<scroll-view @scroll="scroll" :scroll-y="isScrollY" class="drag-and-drop-sort-A">
				<view
					v-for="(item, index) in controlsArray" 
					:key="index" class="_item"
					:style="{'transition': (curretnControlsIndex === index ? 'initial' : '.3s'), 'z-index': (curretnControlsIndex === index ? 1 : 0), 'width': controlsSize.width + 'rpx', 'height': controlsSize.height + 'rpx', 'top': controlsPositionArray[index].top + 'rpx',  'left': controlsPositionArray[index].left + 'rpx'}">
	...
	export default {
		...
		methods: {
			...
			initControlsPosition() {
				...
				let tempArray = [];
				// 每行最大存放的个数
				// this.maxWidthCount = parseInt(this.systemInfo.windowWidth / (this.controlsSize.width + this.margin
				// 	.margin_x));
				this.maxWidthCount = 5
				// 设置控件位置 - 这边多记录一个位置 之后会用到
				for (let i = 0, j = 0, k = 0; i < this.controlsList.length + 1; i++) {
					tempArray[i] = {
						left: (j * (this.controlsSize.width/2 + this.margin.margin_x) + this.margin.margin_x)*2,
						top: (k * (this.controlsSize.height/2 + this.margin.margin_y) + this.margin.margin_y)*2,
					}
			...
			handleTouchmove(event) {
				...
				this.$set(this.controlsPositionArray, this.curretnControlsIndex, {
					left: (this.controlsPositionArray[this.curretnControlsIndex].left/2 + (pageX - this.recordPosition
						.x))*2,
					top: (this.controlsPositionArray[this.curretnControlsIndex].top/2 + (pageY - this.recordPosition
						.y))*2,
				});
				// 记录位置
				this.recordPosition = {
				...
				if (this.curretnControlsIndex + 1 !== this.controlsList.length && (this.curretnControlsIndex + 1) % this
					...
					.width >= this.recordInitControlsPoisitonList[this.curretnControlsIndex].left + this.controlsSize
					.width + this.margin.margin_x + this.controlsSize.width / 2) {
				...
				// 向左移动
						...
						.curretnControlsIndex].left <= this.recordInitControlsPoisitonList[this.curretnControlsIndex - 1]
					.left + this.controlsSize.width / 2 && this.controlsPositionArray[this.curretnControlsIndex].top < this
					.recordInitControlsPoisitonList[this.curretnControlsIndex].top + this.controlsSize.height / 3 + this
					.margin.margin_y && this.controlsPositionArray[this.curretnControlsIndex].top > this
					.recordInitControlsPoisitonList[this.curretnControlsIndex].top - this.controlsSize.height / 3 - this
					.margin.margin_y) {
					this._handleChangeControlsPosition(this.curretnControlsIndex - 1);
				}
				// 向下拖动
				else if (Math.ceil(this.curretnControlsIndex / this.maxWidthCount) !== Math.ceil(this.controlsList.length /
						this.maxWidthCount) && this.controlsPositionArray[this.curretnControlsIndex].top + this
					.controlsSize.height > this.recordInitControlsPoisitonList[this.curretnControlsIndex].top + this
					.controlsSize.height + this.margin.margin_y + this.controlsSize.height / 2) {
					this._handleChangeControlsPosition((this.curretnControlsIndex + this.maxWidthCount) >= this
						.controlsArray.length ? this.controlsArray.length - 1 : this.curretnControlsIndex + this
						.maxWidthCount);
				}
				// 向上拖动
				else if (parseInt(this.curretnControlsIndex / this.maxWidthCount) !== 0 && this.controlsPositionArray[this
						.curretnControlsIndex].top < this.recordInitControlsPoisitonList[this.curretnControlsIndex].top -
					this.margin.margin_y - this.controlsSize.height / 3 * 2) {
